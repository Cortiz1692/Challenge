...
import com.portalInmobiliario.poi_service.config.CacheConfig;
...
public class GeospacialHandler {
    ...
    public GeospacialHandler(GeospatialService geospatialService, PointOfInterestValidator pointOfInterestValidator, POIWebSocketHandler socketHandler, CacheService cacheService) {
    ...
    public Mono<ServerResponse> findPoisNearProperty(ServerRequest request) {
        ...
        return cacheService.getFromCache(cacheKey)
                ...
                .switchIfEmpty(geospatialService.findPoisNearProperty(latitude, longitude, radius).collectList()
        ...
        return poiMono.flatMap(poi -> {
            ...
            if (errors.hasErrors()) {
                return Flux.fromIterable(errors.getFieldErrors())
                        ...
                        .collectList().flatMap(list -> ServerResponse.badRequest().contentType(MediaType.APPLICATION_JSON)
                    ...
                    .flatMap(exists -> {
                        if (exists) {
                            ...
                            return geospatialService.save(poi).flatMap(savedPoi -> ServerResponse
                                    ...
                                    .contentType(MediaType.APPLICATION_JSON).body(BodyInserters.fromValue(savedPoi)));
                        }
                    });
        });
    }
