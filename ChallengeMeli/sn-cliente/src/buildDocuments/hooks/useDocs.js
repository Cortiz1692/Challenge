...
import { addFinalInspectionDate, addTipeShip, findAllDocumentsRejected, findAllDocumentsWithAditional, findAllTipeCertificate, findAllTipeShip, save, saveConvalidationOld, updateStateAditional } from "../services/docService";
...
export const useDocs = () => {
    ...
    const [aditionalDoc, setAditionalDocs] = useState([]);
    ...
    const getDocAditionals = async () => {
        try {
            const result = await findAllDocumentsWithAditional(); // Espera la promesa
            dispatch({
                type: 'loadingDocsAditionals',
                payload: result,
            });
    ...
    const updateAditionalStatus = async ({ idConvalidacion, estado }) => {
        try {
            await updateStateAditional({ idConvalidacion, estado });
            // Manejar la actualización del estado en el contexto o el estado local
        } catch (error) {
            console.error("Error updating additional status:", error);
        }
    };

    const submitInspectionDate = async ({ idCertificado, tipoDocumento, fechaInspeccionAdicional }) => {
        try {

            await addFinalInspectionDate({ idCertificado, tipoDocumento, fechaInspeccionAdicional });

            Swal.fire({
                icon: "success",
                title: "¡Fecha Cargada!",
                text: "La fecha de inspección adicional ha sido cargada correctamente.",
                confirmButtonText: "Aceptar",
            });

        } catch (error) {
            console.error("Error loading additional inspection date:", error);
            Swal.fire({
                icon: "error",
                title: "Error",
                text: "No se pudo cargar la fecha de inspección.",
                confirmButtonText: "Aceptar",
            });
        }
    };
